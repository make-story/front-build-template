...
let getDatetime = () => {
    let date = new Date()
    return [
        [date.getFullYear(), Number(date.getMonth()) + 1, date.getDate()].join(
            ""
        ), // 년월일
...
module.exports = {
    ...
    entry: {
        // IE 환경에서 최신 자바스크립트를 사용해 개발하고 싶다면 두 폴리필을 npm에서 다운 받은 후 저렇게 모든 엔트리에 넣어주셔야 합니다.
    ...
    output: {
        ...
        // css 코드에 url('./assets/bg.png') 파일경로가 존재할 경우, file-loader 를 활용하여 해당 파일읽어 output 경로에 넣어준다.
